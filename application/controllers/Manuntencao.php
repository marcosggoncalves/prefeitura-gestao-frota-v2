...
class Manuntencao extends CI_controller{
	...
	{
		$data['veiculos_servico'] = $this->Dao_manutencao->all_veiculos_status('Serviço');
		$data['veiculos_reserva'] = $this->Dao_manutencao->all_veiculos_status('Reserva');
	...
	public function relatorio_saida_manuntencao(){
        ...
        $config["total_rows"] = $this->Dao_manutencao->count_saida_manuntencao();
        $config["per_page"] = 15;
        $config["uri_segment"] = 2;
        $this->pagination->initialize($config);
		$page = ($this->uri->segment(2)) ? $this->uri->segment(2) : 0;
        $data["links"] = $this->pagination->create_links();
        $data['saida_manuntencao'] = $this->Dao_manutencao->todas_saida_manuntencao($config["per_page"], $page);
	...
	public function salvar_saida_manuntencao(){
		...
		if($this->form_validation->run() == FALSE){
			...
			$save = $this->Dao_manutencao->salvar_saida_manuntencao($campos);
			...
			if($save){
				...
				if( $this->input->post('placa_veiculo') != 'Veiculo não substituido'){
					$this->Dao_manutencao->mudar_status_por_placa($this->input->post('placa_veiculo'),'Indisponivel');
				}

				$this->Dao_manutencao->mudar_status($this->input->post('id_veiculo'),'Indisponivel');
	...
	{
		$data['consulta'] = $this->Dao_manutencao->saida_manuntencao($id);
	...
	public function  saida_manuntencao_deletar($id)
	{
		...
		$this->Dao_manutencao->deletar_saida_manuntencao($id);
	...
	public function alterar_status_saida_manuntencao($id,$status)
	{
		...
		$this->Dao_manutencao->alterar_status_saida_manuntencao($id,$status);
	...
	public function finalizar_saida_manuntencao_salvar($id)
	{
		...
		if($this->form_validation->run() == FALSE){
			...
			$this->Dao_manutencao->mudar_status_por_placa($this->input->post('placa_veiculo'),'Disponivel');
			
			if($this->input->post('substituto_placa_veiculo') != 'Veiculo não substituido'){
				$this->Dao_manutencao->mudar_status_por_placa($this->input->post('substituto_placa_veiculo'),'Disponivel');
			}
			$campos = array(
				'status'=>'fechado',
			...
			$this->Dao_manutencao->finalizar_saida_manuntencao($campos,$id);
